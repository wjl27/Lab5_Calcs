######## Interdiffusion Zone EDS Data ########

rm(list=ls())

library(tidyverse)

data <- as_tibble(read.csv("OverAll_Interdiffusion_Zone_EDS.csv"))
...
n.Ta = length(Ta.dat) # number of samples
n.W = length(W.dat) # number of samples
...
x_bar.g = (1/n.Ta)*sum(Ta.dat) # average hardness
x_bar.h = (1/n.W)*sum(W.dat) # average hardness
...
s_x2.g <- (1/(n.Ta-1))*sum((Ta.dat - x_bar.g)^2) # this is the sample variance of the data set
s_x2.h <- (1/(n.W-1))*sum((W.dat - x_bar.h)^2) # this is the sample variance of the data set
...
s_x.h <- sqrt(s_x2.h) # this is the standard deviation of the data set
...
Sx_bar.g <- s_x.g / sqrt(n.Ta) # standard deviation of means
Sx_bar.h <- s_x.h / sqrt(n.W) # standard deviation of means

El <- c("C","Co","Ni","Pt","Al","Cr","Ta","W")
Wt.Prct <- c(x_bar.a,x_bar.b,x_bar.c,x_bar.d,x_bar.e,x_bar.f,x_bar.g,x_bar.h)
Sx_bar <- c(Sx_bar.a,Sx_bar.b,Sx_bar.c,Sx_bar.d,Sx_bar.e,Sx_bar.f,Sx_bar.g,Sx_bar.h)
N <- c(n.C,n.Co,n.Ni,n.Pt,n.Al,n.Cr,n.Ta,n.W)

results2 <- tibble(El,Wt.Prct,Sx_bar,N)
write.csv(results2, file = "Avg_Interdiffusion_Zone_EDS.csv")